...
| `analysis/stack_scan.py` | does stacking the MA squeeze + volume-profile value width beat either alone? Tests redundancy (corr), the joint surface, and incremental OOS R² — and catches the shared-`range_scale` denominator that inflates one signal. Reuses the indicator-scan cache — self-contained HTML | yes — `commands.bat` → Analysis |
| `algo_metrics/sigma_paper.py` | the learning paper: standard deviation, the bell curve and the 68-95-99.7 rule built from real NQ dailies, where they break (fat tails, measured: 33 four-sigma days vs the 0.3 promised), volatility clustering, and every sigma tool (Bollinger, z-score, VaR, vol targeting, Sharpe) as one recipe — self-contained HTML | yes — `commands.bat` → Analysis → Papers |
| `algo_metrics/four_means.py` | all four textbook means (arithmetic/geometric/harmonic/RMS) on real NQ: GM = AM minus vol drag, RMS = sigma itself, HM = what DCA pays, and the gaps between them all equal half the variance — four PNGs | yes — `commands.bat` → Analysis → Papers |
| `algo_metrics/four_means_paper.py` | the four means dissected piece by piece: each formula as a "flattening" that preserves one property (sum/product/reciprocals/squares), by-hand tables on ten real days, DCA and vol-drag figures, the equal-gap ladder — self-contained HTML | yes — `commands.bat` → Analysis → Papers |
