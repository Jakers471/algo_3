...
|  | `session_efficiency_recent` | 0..1 | yes | range / travel over the last config.RECENT_WINDOW_MINUTES (floored at RECENT_MIN_BARS bars). 1.0 is a straight line; a small number is chop. Needs no range_scale - the ratio is already dimensionless. |
|  | `session_efficiency_prior` | 0..1 | yes | The same measurement, one window further back - the window BEFORE session_efficiency_recent, same width, not overlapping. Comparing this against session_efficiency_recent is how a phase change is caught: efficiency_prior high then efficiency_recent low is an impulse handing off to a base. |
|  | `session_range_ratio` | ratio | yes | recent window's (high-low) span divided by the prior window's. << 1 is contraction - the classic tell that a base is forming, not merely that the market went quiet. |
|  | `session_volume_ratio` | ratio | yes | Sum of volume in the recent window divided by the prior window's. Declining volume through a would-be base is the tell that continuation, not reversal, is next. |
|  | `session_dir_change_rate` | 0..1 | yes | Close-to-close direction reversals in the recent window, / bars in that window. A RATE, not the count session_dir_changes used to be - a monotonic count can only ever grow, so it can never report that the market just started trending; a rate can fall as cleanly as it rises. |
...
|  | `session_val` | price | yes | Value area low: the bottom of the contiguous band around the POC holding config.profile.VALUE_AREA of the session's volume so far. |
|  | `session_vah` | price | yes | Value area high: the top of that same band. |
...
|  | `session_hvn` | payload | detail | Other local volume peaks besides the POC - prices, ascending, each carrying at least HVN_MIN_SHARE of the POC's own volume. What a stop belongs BEHIND, not at the session's tick extreme where resting stops cluster. |
|  | `session_lvn` | payload | detail | Local volume troughs - prices, ascending, each carrying at most LVN_MAX_SHARE of the POC's volume. A target, not an entry: price that reaches one tends to keep moving through it. |
