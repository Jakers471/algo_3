...
- **Keep the mds current after MAJOR changes — not every edit.** A new/renamed/deleted module, a structural change, a new command or workflow → update the affected mds (except `CLAUDE.md`) in the *same commit*, and it's worth running the project audit then too. Minor edits (tweaking a value, a small logic fix, a typo) do NOT need a docs pass — don't churn the docs on every tiny edit. Use judgment; the goal is docs that match reality, not ceremony.
